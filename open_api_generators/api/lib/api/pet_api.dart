...
class PetApi {
  ...
  Future<Response> findPetsByStatusWithHttpInfo({ String? status, }) async {
    // ignore: prefer_const_declarations
  ...
  Future<List<Pet>?> findPetsByStatus({ String? status, }) async {
    final response = await findPetsByStatusWithHttpInfo( status: status, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<List<Pet>?> findPetsByTags({ List<String>? tags, }) async {
    final response = await findPetsByTagsWithHttpInfo( tags: tags, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///
  /// * [MultipartFile] body:
  Future<Response> uploadFileWithHttpInfo(int petId, { String? additionalMetadata, MultipartFile? body, }) async {
    // ignore: prefer_const_declarations
    ...
    if (additionalMetadata != null) {
      queryParams.addAll(_queryParams('', 'additionalMetadata', additionalMetadata));
    }
  ...
  Future<ApiResponse?> uploadFile(int petId, { String? additionalMetadata, MultipartFile? body, }) async {
    final response = await uploadFileWithHttpInfo(petId,  additionalMetadata: additionalMetadata, body: body, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
